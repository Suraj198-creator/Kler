...
PLAN_PRICES = {
    "pro": {
        "price_id": "price_1SL0WCB4otWdZHUzMpB783NO",  # Stripe Price ID
        "amount": 1900,  # $19/month
        "currency": "usd",
        "interval": "month",
        "monthly_credits": 2500  # 2,500 credits/month
    },
    "business": {
        "price_id": "price_1SL0WDB4otWdZHUzBP6KKfAU",  # Stripe Price ID
        "amount": 3900,  # $39/month
        "currency": "usd",
        "interval": "month",
        "monthly_credits": 6000  # 6,000 credits/month
    }
}
...
CREDIT_PACKS = {
    "small": {
        "price_id": "price_1SL0WDB4otWdZHUzOFauQMGv",  # Stripe Price ID
        "amount": 500,  # $5
        "currency": "usd",
        "credits": 500
    },
    "medium": {
        "price_id": "price_1SL0WEB4otWdZHUzWm87fsEP",  # Stripe Price ID
        "amount": 1500,  # $15
        "currency": "usd",
        "credits": 2000
    },
    "large": {
        "price_id": "price_1SL0WEB4otWdZHUzRzfjXRJ4",  # Stripe Price ID
        "amount": 3500,  # $35
        "currency": "usd",
        "credits": 5000
    }
...
async def handle_checkout_completed(session):
    ...
    plan = session['metadata'].get('plan', 'pro')  # Default to pro if not specified

    # Get plan configuration
    plan_config = PLAN_PRICES.get(plan)
    if not plan_config:
        print(f"Invalid plan: {plan}, defaulting to pro")
        plan = 'pro'
        plan_config = PLAN_PRICES["pro"]
